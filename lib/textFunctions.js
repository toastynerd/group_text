...
exports.subscribe = function(req, res, input) {
  ...
  user.save(function(err, mongoRes){
    ...
    res.type('text/xml');
    res.end('<Response><Sms>Thanks for subscribing to Pax Watch text your message to this number to send a message to the list. Text STOPTHEMADNESS to unsubscribe</Sms></Response>');
  });
};

exports.unSubscribe = function(req, res, input) {
  input.splice(input.indexOf('STOPTHEMADNESS'), 1);
